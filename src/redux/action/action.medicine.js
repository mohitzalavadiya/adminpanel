import { AllGetmedicine, DeleteMedicine, PostMedicine, PutMedicine } from "../../common/api/medicineApi";
...
export const datamedicine = () => (dispatch) => {
  try {
    ...
    setTimeout(function () {
      AllGetmedicine()
        .then((data) =>
          dispatch({ type: ActionTypes.DATA_MEDICINE, payload: data.data })
        )
...
export const addmedicine = (data) => (dispatch) => {
  try {
    ...
    PostMedicine(data)
    .then((data) => {
          dispatch({ type: ActionTypes.ADD_MEDICINE, payload: data.data });
        })
        .catch((error) => {
          dispatch(errormedi(error.message));
        });
...
export const updatemedicine = (data) => (dispatch) => {
  try {
    PutMedicine(data)
    .then((data) => {
          dispatch({ type: ActionTypes.UPDATE_MEDICINE, payload: data.data});
        })
        .catch((error) => {
          dispatch(errormedi(error.message));
        });
